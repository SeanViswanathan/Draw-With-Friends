...
class MPCHandler: NSObject, MCSessionDelegate {
    ...
    func setUpBrowser(){
        browser = MCBrowserViewController(serviceType: "My-Draw", session: session)
    }
   
    func advertiseSelf(advertise:Bool){
        if advertise {
            advertiser = MCAdvertiserAssistant(serviceType: "My-Draw", discoveryInfo: nil, session: session)
            advertiser!.start()
        }
